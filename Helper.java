...
public class Helper {
     ...
     */
    static DatacenterBrokerEX createBroker() {
        DatacenterBrokerEX broker = null;
        try {
//			broker = new PowerDatacenterBroker("Broker");
//			broker = new DatacenterBroker("Broker");
            broker = new DatacenterBrokerEX("Broker", -1);
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(0);
        }
    ...
    public static List<Cloudlet> createCloudletList(int brokerId, int cloudletsNumber) {
        ...
        long fileSize = 300;
        long outputSize = 300;
        long seed = Constants.CLOUDLET_UTILIZATION_SEED;
        UtilizationModel utilizationModelNull = new UtilizationModelNull();

		Random ran = new Random(Constants.CLOUDLET_LENGTH_SEED);
        ...
        for (int i = 0; i < cloudletsNumber; i++) {
            ...
            if (seed == -1) {
                ...
                cloudlet = new Cloudlet(
                        i,
						Constants.CLOUDLET_LENGTH * (ran.nextInt(Constants.CLOUDLET_LENGTH_BOUND) + 1),
//                        Constants.CLOUDLET_LENGTH,
                        Constants.CLOUDLET_PES,
                        fileSize,
                        outputSize,
//                        new UtilizationModelFull(),
       ...
       //     cloudlet.setVmId(ran.nextInt(200));
            list.add(cloudlet);
        }
